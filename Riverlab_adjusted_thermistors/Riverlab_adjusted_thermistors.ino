...
#define READ_INTERVAL 15                           // read interval, in minutes
#define SEND_INTERVAL 12                           // telemetry interval, in hours
#define NREADINGS 25                              // number of readings taken per measurement
...
RioLogger myLogger = RioLogger();
    ...
    DallasTemperature sensors(&oneWire); // for thermometer on the board or digital thermometer??
...
#define THERMISTORPIN0 A0 // change pin if needed
#define Bcoef     //calibration parameters ; match with T0-10
#define intercept     //calibration parameters ; match with T0-10
...
//measure to resistance
 ...
 float calT0;
calT0 = intercept +Bcoef *log(average) // calibrated formula - replaces Steinhart
...
//sd card related stuff:
    ...
    SdFat SD;
    SdFile dataFile;
    char filename[] = "readings.CSV"; //changed into 'readings'
    boolean SDcardOn;
    byte keep_SPCR;
    uint8_t status;
...
void loop () 
{
    ...
    //    #endif
        ...
        // measure thermistor
   float Temp_25s = MeasureTemp( NUMSAMPLES_25, THERMISTORPIN0);
